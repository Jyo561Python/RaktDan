...
<MDBottomNavigationHeader>
    ...
    width:
        ((root.panel.width*.9-dp(60)) / len(root.panel.ids.tab_manager.screens)) \
        if len(root.panel.ids.tab_manager.screens) != 0 else root.panel.width*.9
...
class MDBottomNavigationItem(MDTab):
    ...
    def on_tab_press(self, *args):
        ...
        par.ids.tab_manager.current = self.name
        if par.previous_tab is not self:
            Animation(scale=1, d=0.1).start(par.previous_tab.header)
            Animation(
